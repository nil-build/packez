...
_commander.default.version(_package.default.version).option("-w, --watch", "是否监控文件改变").option("-d, --outDir [outDir]", "输出到指定目录，默认为 dist", "dist").option("-t, --target [target]", "转换目标格式：web | node 默认为 web", /^node|web$/, "web").option("-c, --clear", "转换前清空输出目录").option("-p, --publicPath [publicPath]", "publicPath", "").option("-l, --loaders [loaders]").option("--config [config]", "configFile", "./packez.config.js").option("--cnpm", "使用cnpm安装依赖").option("--corejs [corejs]", "参考 babel-runtime", false).option("--helpers [helpers]", "参考 babel-runtime", false).option("--regenerator [regenerator]", "参考 babel-runtime", false).parse(process.argv);
...
if (args[0] === "init") {
  ...
  _fsExtra.default.writeFileSync(pkgFile, (0, _stringify.default)(userPkg, null, 2) + _os.default.EOL);
