...
module.exports = function (cfg) {
  ...
  const plugins = [new webpack.IgnorePlugin(...(cfg.ignore || cfg.IgnorePlugin)), new webpack.DefinePlugin(cfg.defines || cfg.DefinePlugin)];

  if (cfg.banner || cfg.BannerPlugin) {
    plugins.push(new webpack.BannerPlugin(cfg.banner || cfg.BannerPlugin));
  } //开启manifest模式
  ...
  if (cfg.cleanDist) {
    ...
    plugins.push(new CleanWebpackPlugin(path.basename(cfg.appDist), {
      root: cfg.appPath
    }));
  } //打包合并css成文件
  ...
  if (cfg.appEntryHtml) {
    ...
    plugins.push(new HtmlWebpackPlugin((0, _assign.default)({}, cfg.appEntryHtmlOpts, htmlOpts)));
  }
